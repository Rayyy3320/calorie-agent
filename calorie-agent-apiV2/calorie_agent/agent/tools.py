...
def _food_database_for_user(message: dict[str, str], legacy: ModuleType) -> list[dict[str, Any]]:
    return aliases.apply_user_aliases(legacy._load_food_database(), message, legacy=legacy)
...
def _tool_create_food(
    ...
    food = legacy._create_food_database_record(_food_name(action), per_100g)  # type: ignore[arg-type]
